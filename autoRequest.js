...
var requestAuto = function(queryRequest){
	try {
		...
		EqualsOffer.prototype.requetEmpty = (network, query, db) =>{
		...
		EqualsOffer.prototype.findLinkAPI = (querySearchEmpty) =>{
			mongo.connect(pathMongodb, (err, db)=>{
				...
				db.collection("userlist").findOne(query, (err, result)=>{
					equalsOffer.requetEmpty(result.NetworkList, querySearchEmpty, db)
				})
			})
		}
		equalsOffer.findLinkAPI(queryRequest);
	} catch(e) {
		console.log(e);
	}
}
